...
<body>
	...
	$i=0;
	$reponse = $bdd->query("SELECT Etat, Numero_Terrain, Equipe_1, Equipe_2, Score_Equipe_1, Score_Equipe_2 FROM matchs WHERE ID_Tournoi = '".$id_tournoi[0]."' ORDER BY `matchs`.`Etat` DESC");
	while ($donnees = $reponse->fetch()) {
		 $etat[$i] = $donnees['Etat'];
		 $num_terrain[$i] = $donnees['Numero_Terrain'];
	...
	echo "<section class='tournoi_parent'>
		...
		<p><label>En cours</label><br><br>" . $_POST['tournoi_intitule'] . " / " . $_POST['tournoi_sport'] . "</p>
	...
	for ($i=0; $i<count($equipe1); $i++) {
		if ($etat[$i]=="TERMINE") {
			...
			{
				echo "<strong class='potit_match'>MATCH ".($i+1)." - TERMINÉ<br>Terrain ".$num_terrain[$i]."</strong><br>
		...
		else if ($etat[$i]=="EN_COURS") {
			echo "<strong class='potit_match_active'>MATCH ".($i+1)." - EN COURS</strong><br><strong class='potit_match'>Terrain ".$num_terrain[$i]."</strong><br>
			<section class=sect_inline_equipe1_active>
				<label>".$equipe1[$i]."</label>
			</section>
			<section class=sect_inline_score1_active>
				<label>". $score_equipe1[$i]."</label>
			</section>
			-
			<section class=sect_inline_score2_active>
				<label>". $score_equipe2[$i]."</label>
			</section>
			<section class=sect_inline_equipe2_active>
				<label>".$equipe2[$i]."</label>
			</section><br><br>";
		}
		else if ($etat[$i] == "A_VENIR") {
			echo "<strong class='potit_match_incoming'>MATCH ".($i+1)." - À VENIR<br>Terrain ".$num_terrain[$i]."</strong><br>
			<section class=sect_inline_equipe1_incoming>
				<label>".$equipe1[$i]."</label>
			</section>
			<section class=sect_inline_score1_incoming>
				<label>". $score_equipe1[$i]."</label>
			</section>
			-
			<section class=sect_inline_score2_incoming>
				<label>". $score_equipe2[$i]."</label>
			</section>
			<section class=sect_inline_equipe2_incoming>
				<label>".$equipe2[$i]."</label>
			</section><br><br>";
		}
	...
	echo "</section>
		...
		echo "<br><br><br><br><br><h2>Aucun match trouvé...</h2>";
	}
